...
# Liste alle Bilddateien im Verzeichnis auf
image_files = [f for f in os.listdir(out_img_dir)]

# Erstes Bild laden, um die Größe zu erhalten und als Basis zu verwenden
if not image_files:
    print("Keine Bilder im Verzeichnis gefunden!")
    exit()

# Erstes Bild als Basis laden
base_image_path = os.path.join(out_img_dir, image_files[0])
result_image = cv2.imread(base_image_path, cv2.IMREAD_GRAYSCALE)

# Über alle verbleibenden Bilder iterieren und mit "Multiplizieren"-Modus überblenden
for image_file in image_files[1:]:
    image_path = os.path.join(out_img_dir, image_file)
    image = cv2.imread(image_path, cv2.IMREAD_GRAYSCALE)

    # Konvertiere die Bilder zu float und normalisiere sie auf den Bereich 0-1
    result_float = result_image.astype(float) / 255.0
    image_float = image.astype(float) / 255.0

    # Überblendungs-Modus "Multiplizieren" anwenden
    result_float = result_float * image_float

    # Zurück zu 8-bit konvertieren
